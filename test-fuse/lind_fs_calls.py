...
Currently my file system is doing all the work of creating and deleting files and Directories

But somehow the CAT of a file is not outputting the return value of read systemcall onto the terminal
If you see the output in the terminal you can see the return value of read. But the flush is not flusing it to the terminal
...
#      They are generated sequentially. - This idea has to be changed 
...
def _blank_fs_init():
  ...
  # Now setup blank data structures
	...
	#- This needs to be dynamically taken. So I am not initializing it here instead dealing in open and mkdir
  #filesystemmetadata['nextinode'] = 30 
  ...
  fastinodelookuptable['/'] = ROOTDIRECTORYINODE
	...
	# instead of persisiting this I need to recursively build it on filesystem reloading

  persist_metadata(METADATAFILENAME)
	
	# for the time being I am leaving the above 

  #The super block will be linddata.0 and consists of the following data
  superblock['creationTime'] = 1376483073
  superblock['mounted'] = 1
  superblock['devId'] = 20
  superblock['freeStart'] = 1
  superblock['freeEnd'] = 25
  superblock['root'] = ROOTDIRECTORYINODE
  superblock['maxBlocks'] = 10000

  #I need to persist this in linddata.0
  persist_superblock(SUPERBLOCKFILENAME)
  
  #Now I need to create the freeblocks and store it in files from 1 - 25
  temp = 30
  tempDict = {}  
  for x in range(1, 25):
    for y in range (1, 400):
      tempDict[temp] = FILEDATAPREFIX + str(temp)
      temp += 1
    #The key value pairs consists of block number and the name of the file that block can be found
    #This function stores the data in a serialized format in the file FILEDATAPREFIX + (1,25)
    storefreeblocks(x, tempDict)
    #need to clear the dictionary for the next pass
    tempDict.clear()
  #Now we have 25 files with linddata.X (1, 25) with 400 pointers each

# These are used to initialize and stop the system
def storefreeblocks(x, tempDict):
  freeblockdata = serializedata(tempDict)
  try:
    removefile(FILEDATAPREFIX + str(x))
  except FileNotFoundError:
    pass
  freeblockfo = openfile(FILEDATAPREFIX+str(x), True)
  log(FILEDATAPREFIX + str(x))
  freeblockfo.writeat(freeblockdata, 0)
  freeblockfo.close()  

#This will be removed at a later time
# Persists the SUPER block in linddata.0
def persist_superblock(superblockfilename):
  superblockstring = serializedata(superblock)
  try:
    removefile(superblockfilename)
  except FileNotFoundError:
      pass
  superblockfo = openfile(superblockfilename, True)
  superblockfo.writeat(superblockstring,0)
  superblockfo.close()

# author: kidd0
# This function is used to persist a  dictionary into a given block

def persist_block(tempDict, block_no):
  tempBlockData = serializedata(tempDict)
  try:
    removefile(FILEDATAPREFIX + str(block_no))
  except FileNotFoundError:
      pass
  blockfilefo = openfile( FILEDATAPREFIX + str(block_no), True)
  blockfilefo.writeat(tempBlockData,0)
  blockfilefo.close()




#Done: This function has to be moved to the helper functions later on

#TODO: How about changing this function to return just one free block and calling this recursively??
#TODO: May be if this part doesn't work properly
def allocate_freeblocks(no_of_blocks_required):
	if no_of_blocks_required < 1 or no_of_blocks_required > 400 :
		raise Exception("Max allowed file system size is 1638400")

	free_block_dict = {}
	for i in range(1, 25):
		if no_of_blocks_required == 0 :
			break
		else:
			#open a linddata.X file
			currlinddatafo = openfile(FILEDATAPREFIX+str(i), True)
			#deserialize the data
			freeblockdata = currlinddatafo.readat(None, 0)
			currlinddatafo.close()			
			# get the dict we want
			desiredfreeblockdata = deserializedata(freeblockdata)
			#find number of blocks available
			no_of_blocks_available = len(desiredfreeblockdata)
			if ( no_of_blocks_available >= no_of_blocks_required):
				# we can allocate the free blocks from this 
				while no_of_blocks_required > 0:
					block_no, block_file = desiredfreeblockdata.popitem()
					free_block_dict[block_no] = block_file
					no_of_blocks_required -= 1
					

			else:
				# assign all the  available free blocks and continue finding free blocks
				while no_of_blocks_available > 0:
					block_no, block_file = desiredfreeblockdata.popitem()
					free_block_dict['block_no'] = block_file
					no_of_blocks_available -= 1
		
			# Update the linddata.X file so that the allocated free blocks are removed - Done since the popitem() is used
			
			#Serialize and store the data again 
			updatedfreeblockdata = serializedata(desiredfreeblockdata)
			try:
				removefile(FILEDATAPREFIX + str(i))
			except FileNotFoundError:
				pass
			freeblockfo = openfile(FILEDATAPREFIX+str(i), True)
			freeblockfo.writeat(updatedfreeblockdata, 0)
			freeblockfo.close()  

	# This check is required since we may not find the required number of blocks even after looping the 25 files 
	if no_of_blocks_required == 0:
		#return a dictionary of freeblocks
		return free_block_dict

	else:
		# Error file system doesnt have the required space
		raise Exception("File system doesn't have the required number of Blocks to allocate")		


def deallocate_freeblock(block_no):
	try:
		removefile(FILEDATAPREFIX + str(block_no))
	except FileNotFoundError:
		pass
	...
	for i in range(1,25):
		...
		#find number of blocks available
		no_of_blocks_available = len(desiredfreeblockdata)
		if no_of_blocks_available > 400:
			pass
		else:
			desiredfreeblockdata[block_no] = FILEDATAPREFIX+ str(block_no)
			updatedfreeblockdata = serializedata(desiredfreeblockdata)
			try:
				removefile(FILEDATAPREFIX + str(i))
			except FileNotFoundError:
...
def mkdir_syscall(path, mode):
  ...
  try:
    ...
    free_block_inode_no, free_block_file_name = allocate_freeblocks(1).popitem()

    newinode = free_block_inode_no		#filesystemmetadata['nextinode']
    ...
    filesystemmetadata['inodetable'][parentinode]['linkcount'] += 1
		# All the changes made to the parent directory have to be persisted.
    parentDictionary = filesystemmetadata['inodetable'][parentinode]
    # finally, update the fastinodelookuptable and return success!!!
    fastinodelookuptable[truepath] = newinode

    persist_block(newinodeentry, newinode)
    persist_block(parentDictionary, parentinode)
  ...
  try:
    ...
    try:
      ...
      raise SyscallError("rmdir_syscall","EEXIST","The path does not exist.**")
		...
		# This inode should be added back to the free inodes 
    deallocate_freeblock(thisinode)
		#The parent block inode file should be updated with the changes
    persist_block(filesystemmetadata['inodetable'][parentinode], parentinode)
    ...
    return 0
		...
		# The directory information stored in a block should be removed first.
...
def fstat_syscall(fd):
  ...
  inode = filedescriptortable[fd]['inode']
	#TODO: This line is changed from 0,1,2 to this. Mostly needs to be changed to include all inodes till 29
  if fd in [0, 1, 2] or \
  ...
  try:
    ...
    if truepath not in fastinodelookuptable:
      ...
      if not O_CREAT & flags:
				# this means that the open is called without the CREAT flag and since the file doesnt exist (not in fastinodelookuptable) raise error
        raise SyscallError("open_syscall","ENOENT","The file does not exist.")
      ...
      if trueparentpath not in fastinodelookuptable:
				# Parent should be in the fastinodelookuptable
      ...
      temp_inode_no, temp_inode_file = allocate_freeblocks(1).popitem()
      temp_location_no, temp_location_file = allocate_freeblocks(1).popitem()
      newinode = temp_inode_no #allocate_freeblocks(1).popitem()#filesystemmetadata['nextinode']
      #filesystemmetadata['nextinode'] += 1
      ...
      newinodeentry = {'size':0, 'uid':DEFAULT_UID, 'gid':DEFAULT_GID, 
            'mode':effective_mode,
            'atime':1323630836, 'ctime':1323630836, 'mtime':1323630836,
						'indirect':0, 'location': temp_location_no,
            'linkcount':1}
    
      # ... and put it in the table..
      filesystemmetadata['inodetable'][newinode] = newinodeentry
			# Persist the inode
      inode_string = serializedata(newinodeentry)			
      try:
			  removefile(temp_inode_file)
      except:
			  pass
      temp_inode_fo = openfile(temp_inode_file, True)
      temp_inode_fo.writeat(inode_string, 0)
      temp_inode_fo.close()
			# Done with persisting the inode related data. Time to update the parent's Inode
      ...
      fastinodelookuptable[truepath] = newinode
			...
			# Time to persist the parents inode into the file
      try:
	    	removefile(FILEDATAPREFIX+str(parentinode))
      except:
        pass
      parent_inode_fo = openfile(FILEDATAPREFIX+str(parentinode), True)
      parent_inode_info = serializedata(filesystemmetadata['inodetable'][parentinode])
      parent_inode_fo.writeat(parent_inode_info, 0)
      parent_inode_fo.close()
      ...
      if O_TRUNC & flags:
        ...
        if inode in fileobjecttable:
          ...
          #filesystemmetadata['inodetable'][inode]['size'] = 0
					# Persist the size of the file as 0
          inode_metadata_dict = filesystemmetadata['inodetable'][inode]
#					'location': temp_location_no
          inode_data_pointer = inode_metadata_dict['location']
          inode_metadata_str_ds = serializedata(inode_metadata_dict)
          try:
						removefile(FILEDATAPREFIX+str(inode))
						removefile(FILEDATAPREFIX+str(inode_data_pointer))
          except:
						pass
          inode_metadata_fo = openfile(FILEDATAPREFIX+str(inode), True)
          inode_metadata_fo.writeat(inode_metadata_str_ds, 0)
          inode_metadata_fo.close()

        # always open the file.
				# This point is where I am not sure if I need to make changes - kidd0
    ...
    if IS_REG(filesystemmetadata['inodetable'][inode]['mode']):
      # this is a regular file.  If it's not open, let's open it! 
			# I think its a redundant check. SInce both the control stmnts open the file.
...
def read_syscall(fd, count):
  ...
  try:
    ...
    position = filedescriptortable[fd]['position']
		# kidd0-Edits start here 
		#inode contains the metadata 

    metadata_dict = filesystemmetadata['inodetable'][inode]
    data_to_return = ""
    addressing_mode = metadata_dict['indirect']
    if (addressing_mode == 0):
			# File is < 4096 bytes hence read can be done by opening just one file
			file_data_block = metadata_dict['location']
			file_data_fo = openfile(FILEDATAPREFIX+str(file_data_block),True)
			actual_file_data = file_data_fo.readat(None,0)
			file_data_fo.close()
			data_to_return += actual_file_data[position:count+position]			
    else:
			# File is > 4096 get to indirect block read all the content then talk to me
			index_block_no = metadata_dict['location']
			index_block_fo = openfile(FILEDATAPREFIX+str(index_block_no), True)
			index_block_serialized_dict = index_block_fo.readat(None,0)
			index_block_dict = deserializedata(index_block_serialized_dict)
			print "the data content is in the following blocks:"
			print index_block_dict
			actual_file_data = ""
			for block_no,block_file_name in index_block_dict.iteritems():
				temp_fo = openfile(FILEDATAPREFIX+str(block_no), False)
				actual_file_data = actual_file_data+str(temp_fo.readat(None,0))
				temp_fo.close()
			data_to_return += actual_file_data[position: count+position]
...
#    data = fileobjecttable[inode].readat(count,position)
    ...
    filedescriptortable[fd]['position'] += len(data_to_return)
    print "-------data returned by read system call ------------"		
    print data_to_return
    print "-----------------------------------------------------"
...
### Helper function for write - need to move this to the helper functions
"""
	write a func which take str - computes no_blocks req - writes the str to those - return the block no's as dict				
	contents_of_new_location = write_data_to_multiple_blocks(data_to_write)
"""
def write_data_to_multiple_blocks(data_to_write):

  len_of_data = len(data_to_write)
  no_of_blocks_req = len_of_data / float(4096)
  #logic for rounding off the blocks 
  temp_val = no_of_blocks_req - int(no_of_blocks_req)
  if (temp_val > 0):
    # we need to add 1 to the int(no_of_blocks_req)
    no_of_blocks_req = int(no_of_blocks_req) + 1
  else:
    no_of_blocks_req = int(no_of_blocks_req)
	
  data_blocks_dict = {}
	# now we have the correct number of blocks that are required
  curr_position = 0
  for i in range(0,no_of_blocks_req):
    # get one free block, add that to dict, write 4096 bytest to that file ,persist this
    free_block_inode_no, free_block_file_name = allocate_freeblocks(1).popitem()
    data_blocks_dict[free_block_inode_no] = free_block_file_name
    # each time we write 4096 bytes
    temp_str = data_to_write[curr_position:curr_position+4096]
    # write this into the file
    # Should i check if the file exist? not necessary I guess
    data_blck_fo = openfile(FILEDATAPREFIX+str(free_block_inode_no), True)
    data_blck_fo.writeat(temp_str, 0)
    data_blck_fo.close()
    curr_position += 4096
...
def write_syscall(fd, data):
  ...
  """
	# filedescriptortable['inode'] - will have the inode to the file - which is nothing but the metadata of the file to be written 
  ...
  try:
    ...
    if not IS_REG(filesystemmetadata['inodetable'][inode]['mode']):
      raise SyscallError("write_syscall","EINVAL","File descriptor does not refer to a regular file.")
		...
		# This inode has the metadata for the file - Start by reading the metadata file

    # let's get the position...
    position = filedescriptortable[fd]['position']
    inode_metadata = filesystemmetadata['inodetable'][inode]
    file_addressing_mode = inode_metadata['indirect']
    current_file_size	= inode_metadata['size']
    if (file_addressing_mode == 0):
  		# Currently the file is < 4096 bytes and hence indirect is 0
			sizeof_data_to_write = len(data) + current_file_size
			if ( sizeof_data_to_write < 4096):
				# No need to change addressing_mode - Just carry on with writing the data
				# For this we need to open the file which has the data
				filesize = sizeof_data_to_write
				data_block_no = inode_metadata['location']
				data_block_fo = openfile(FILEDATAPREFIX+str(data_block_no), True)
				existing_data = data_block_fo.readat(None,0)
				data_block_fo.close()
				try:
					removefile(FILEDATAPREFIX+str(data_block_no))
				except:
					raise SyscallError("write_syscall","EBADF","The already existing data file is corrupted or doesn't exist")
				data_block_new_fo = openfile(FILEDATAPREFIX+str(data_block_no), True)
				data_block_new_fo.writeat(existing_data+str(data), 0)
				data_block_new_fo.close()
			else:
				# Currently the file is < 4096 bytes But after adding the data its > 4096
				# Now we need to change the indirect to 1 and then turn location to index blocks 
				# Edit:: Lets write a helper function which performs the real write to block files 
				
				#lets begin by extacting the already existing data.
				data_block_no = inode_metadata['location']
				data_block_fo = openfile(FILEDATAPREFIX+str(data_block_no), True)
				existing_data = data_block_fo.readat(None,0)
				data_block_fo.close()
				data_to_write = existing_data + data
				filesize = len(data_to_write)
				# write a func which take str - computes no_blocks req - writes the str to those - return the block no's as dict				
				contents_of_new_location = write_data_to_multiple_blocks(data_to_write)
				
				# change the addressing mode
				inode_metadata['indirect'] = 1
				# persist the list of blocks in which the data currently exist in the location block				
				try:
					removefile(FILEDATAPREFIX+str(data_block_no))
					inode_metadata['size'] = current_file_size+len(data)
				except:
					pass
				# serialize the index block contents
				index_block_contents_serialized = serializedata(contents_of_new_location)
				new_index_block_fo = openfile(FILEDATAPREFIX+str(data_block_no),True)
				new_index_block_fo.writeat(index_block_contents_serialized,0)
				new_index_block_fo.close()
				
				# now persist the changes of the file_metadata 				
				new_metadata_serialized = serializedata(inode_metadata)
				try:
					removefile(FILEDATAPREFIX+str(inode))
				except FileInUseError:
					temp_fo = fileobjecttable[inode]
					temp_fo.close()
					pass

				metadata_fo = openfile(FILEDATAPREFIX+str(inode),True)
				metadata_fo.writeat(new_metadata_serialized, 0)		
				fileobjecttable[inode] = metadata_fo
				#metadata_fo.close()
        # done - write the helper function and the else case condition
    else:
				# Currently the file is > 4096 bytes and hence indirect is 1
				# Edit:: Lets write a helper function which performs the real write to block files 
				
				#lets begin by extacting the already existing data.
				index_block_no = inode_metadata['location']
				index_block_fo = openfile(FILEDATAPREFIX+str(index_block_no), False)
				index_block_contents = deserializedata(index_block_fo.readat(None,0))
				index_block_fo.close()

 				# need to loop over these blocks and get the existing content
				existing_data = ""
				for data_block_no,data_block_file in index_block_contents.iteritems():
					data_block_fo_temp = openfile(data_block_file, False)
					existing_data += str(data_block_fo_temp.readat(None, 0))
					data_block_fo_temp.close()
				# - finished getting the existing data			
				data_to_write = existing_data + data
				filesize = len(data_to_write)
				contents_of_new_location = write_data_to_multiple_blocks(data_to_write)
				# persist the list of blocks in which the data currently exist in the location block				
				try:
					removefile(FILEDATAPREFIX+str(index_block_no))
					inode_metadata['size'] = len(contents_of_new_location)
				except FileInUseError:
					pass
				# serialize the index block contents
				new_index_block_serialized = serializedata(contents_of_new_location)
				#index_block_contents_serialized = serializedata(contents_of_new_location)  ---- need to make changes
				new_index_block_fo = openfile(FILEDATAPREFIX+str(index_block_no),True)
				new_index_block_fo.writeat(new_index_block_serialized,0)
				new_index_block_fo.close()
...
def ftruncate_syscall(fd, new_len):
  ...
  try: 
    ...
    addressing_mode = filesystemmetadata['inodetable'][inode]['indirect']
    filesize = filesystemmetadata['inodetable'][inode]['size']
    location_block = filesystemmetadata['inodetable'][inode]['location']
    if (addressing_mode == 0):
      if filesize < new_len:
        # we must pad with zeros
        blankbytecount = new_len - filesize 
        openfile(FILEDATAPREFIX+str(location_block),True).writeat('\0'*blankbytecount, filesize)
      
      else:
        # we must cut
        to_persist = openfile(FILEDATAPREFIX+str(location_block),True).readat(new_len,0)
        #openfile(FILEDATAPREFIX+str(location_block),True).writeat(to_persist,0)
    else:
      index_block = filesystemmetadata['inodetable'][inode]['location']      
      index_block_dict_deserialized = openfile(index_block, True).readat(None,0)
      index_block_dict = deserializedata(index_block_dict_deserialized)
